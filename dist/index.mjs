    ...
    if (parsed && typeof parsed === "object" && parsed.endpoint && parsed.method && parsed.params !== void 0) {
      ...
      try {
        ...
        if (parsed.method === "GET") {
          apiResponse = yield axios.get(url, { params: parsed.params });
        } else if (parsed.method === "POST") {
          apiResponse = yield axios.post(url, parsed.params);
        }
  ...
  return __async(this, null, function* () {
    ...
    if (from === "ETH" && to !== "ETH") {
      ...
      const amountOutMin = new BN(amountOut).sub(
        new BN(amountOut).mul(new BN(Math.floor(slippage * 1e3))).div(new BN(1e3))
      );
...
function createSwapHandler() {
  return (req, res) => __async(null, null, function* () {
    var _a;
    const body = req.body || (yield (_a = req.json) == null ? void 0 : _a.call(req));
    const result = yield swapHandler(body);
    if (res) {
      res.json ? res.json(result) : res.end(JSON.stringify(result));
    } else {
      return result;
...
function transferHandler(body) {
  return __async(this, null, function* () {
    ...
    if (token.toUpperCase() === "ETH") {
      ...
      if (new BN2(balance2).lt(new BN2(amountInWei))) {
    ...
    try {
      const [whole, dec] = String(amount).split(".");
      const wholeBN = new BN2(whole || "0").mul(new BN2(10).pow(new BN2(tokenMeta.decimals)));
      let decBN = new BN2(0);
      if (dec) {
    ...
    if (new BN2(balance).lt(amountInBaseUnit)) {
...
function createTransferHandler() {
  return (req, res) => __async(null, null, function* () {
    var _a;
    const body = req.body || (yield (_a = req.json) == null ? void 0 : _a.call(req));
    const result = yield transferHandler(body);
    if (res) {
      res.json ? res.json(result) : res.end(JSON.stringify(result));
    } else {
      return result;
...
export {
  ...
  getTokenMeta,
  handleMindcoreAnalyst,
  handleMindcoreTraderCopilot,
  swapHandler,
  transferHandler
};
